...
void AB_init(void)
{
	...
	//AB_SPI.CTRL = SPI_CLK2X_bm | SPI_MASTER_bm | SPI_MODE_0_gc | SPI_PRESCALER_DIV4_gc;
	//AB_SPI.INTCTRL = 0;
	//AB_SPI.CTRL |= SPI_ENABLE_bm;
	
	AB_USART.CTRLA = 0;
	AB_USART.CTRLB = USART_TXEN_bm;
	AB_USART.CTRLC = USART_CMODE_MSPI_gc | USART_CHSIZE_8BIT_gc;
	AB_USART.BAUDCTRLA = AB_USART_BSEL & 0xFF;
	AB_USART.BAUDCTRLB = ((AB_USART_BSCALE << 4) & 0xF0) | ((USART_BSEL >> 8) & 0x0F);
