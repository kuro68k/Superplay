...
{
	KBUS_DMA_CH.CTRLA &= ~DMA_CH_ENABLE_bm;
	while(KBUS_DMA_CH.CTRLA & DMA_CH_ENABLE_bm);
	
	KBUS_DMA_CH.TRFCNTL = sizeof(KBUS_PACKET_t);
	KBUS_DMA_CH.REPCNT = 1;
	KBUS_DMA_CH.DESTADDR0 = (( (uint16_t)&input_buffer) >> 0) & 0xFF;
	KBUS_DMA_CH.DESTADDR1 = (( (uint16_t)&input_buffer) >> 8) & 0xFF;
	KBUS_DMA_CH.DESTADDR2 = 0;
}
...
void KBUS_init(void)
{
	...
	// DMA from USART
	KBUS_DMA_CH.CTRLA = DMA_CH_RESET_bm;
	NOP();
	KBUS_DMA_CH.CTRLA = DMA_CH_BURSTLEN_1BYTE_gc;
	KBUS_DMA_CH.CTRLB = 0;
	KBUS_DMA_CH.ADDRCTRL = DMA_CH_SRCRELOAD_NONE_gc | DMA_CH_SRCDIR_FIXED_gc | DMA_CH_DESTRELOAD_BLOCK_gc | DMA_CH_DESTDIR_INC_gc;
	KBUS_DMA_CH.TRIGSRC = KBUS_DMA_TRIGGER_SRC;
	KBUS_DMA_CH.SRCADDR0 = (( (uint16_t)&KBUS_USART.DATA) >> 0) & 0xFF;
	KBUS_DMA_CH.SRCADDR1 = (( (uint16_t)&KBUS_USART.DATA) >> 8) & 0xFF;
	KBUS_DMA_CH.SRCADDR2 = 0;
	kbus_reset_dma();
	...
	KBUS_TC.CTRLB = 0;
	KBUS_TC.CTRLC = 0;
	KBUS_TC.CTRLD = TC_EVACT_RESTART_gc | KBUS_EVSEL;
	KBUS_TC.CTRLE = 0;
	KBUS_TC.INTCTRLA = TC_OVFINTLVL_LO_gc;
	KBUS_TC.INTCTRLB = 0;
	KBUS_TC.CNT = 0;
	KBUS_TC.PER = 0x9C3F;	// timeout, 10mS @ 16MHz/4
	KBUS_TC.CCA = 0x004F;	// end of packet, 20uS @ 16MHz/4
	KBUS_TC.CTRLA = TC_CLKSEL_DIV4_gc;
}
...
{
	uint8_t	byte = KBUS_USART.DATA;	// also clears ISR flag
	
	// check for overflows
	if (input_write_ptr_AT >= sizeof(KBUS_PACKET_t))
		input_write_ptr_AT = 0;
	
	//uint8_t	* const buffer = (uint8_t *)&input_buffer_AT;
	//buffer[input_write_ptr_AT++] = byte;
	*((uint8_t *)&input_buffer_AT + input_write_ptr_AT++) = byte;

	if (input_write_ptr_AT > 4)		// minimum packet size = 4 bytes
	{
		if (input_write_ptr_AT == (input_buffer_AT.length + 4))		// packet reached correct size
		{
			packet_ready_SIG = 0xFF;
			input_write_ptr_AT = 0;
		}
	...
	for(;;)
	{
		...
		if (packet_ready_SIG)
		{
			...
			if (input_buffer.command == (CMD_READ_REPORT | RESPONSE_BIT_bm))
			{
				...
				// handle packet
				SAT_update((REPORT_t *)&packet.data);
			}
		}
	}
}
