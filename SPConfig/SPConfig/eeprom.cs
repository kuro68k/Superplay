...
namespace SPConfig
{
	class eeprom
	{
		private static UInt16[] prescalers = { 1, 2, 4, 8, 64, 256, 1024 };
		private const UInt16 CONFIG_ID = 0x2768;
		private const UInt32 CPU_FREQ = 16000000;
		private const UInt32 PER_MAX = 0xFFFF;


		[StructLayout(LayoutKind.Sequential, Pack = 1)]
		public struct eep_config
		{
			public UInt16 config_id;					// must be CONFIG_ID
			public UInt16 config_size;

			public byte button_mode;

			public byte af_mode;
			public byte af_high_05hz;
			public byte af_low_05hz;
			public UInt16 af_mask;
			public byte af_save_state;

			public byte rotary_mode;

			public byte start_af_toggle_mode;

			// 14 bytes of padding
			public UInt32 padding0;	// 4
			public UInt32 padding1;	// 8
			public UInt32 padding2;	// 12
			public UInt16 padding3;	// 14
		...
		private static bool calc_timer(double freq, out byte prescaler, out UInt16 per)
		{
			UInt32 cycles;
			UInt32 lowest_error = 0xFFFFFFFF;
			byte best_prescaler = 0xFF;
			UInt16 best_per = 0;

			cycles = (UInt32)(CPU_FREQ / freq);

			// find best prescaler
			for (byte i = 0; i < prescalers.Count(); i++)
			{
				UInt32 test_per;

				test_per = cycles / prescalers[i];
				test_per--;
				if (test_per > PER_MAX)	// unusable
					continue;

				UInt32 error;
				error = (UInt32)Math.Abs((Int32)cycles - (Int32)((test_per + 1) * prescalers[i]));
				if (error < lowest_error)
				{
					lowest_error = error;
					//best_prescaler = (Int16)prescalers[i];
					best_prescaler = i;
					best_per = (UInt16)test_per;
				}
			}

			if ((best_prescaler == 0xFF) || (best_per == 0))	// failed to find a suitable value
			{
				prescaler = 7;	// 15Hz safe setting
				per = 15625;
				return false;
			}
		...
		private static UInt32 crc32(byte[] buffer, int size)
		{
			var table = new uint[]{
